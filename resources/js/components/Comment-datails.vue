<template>
    <div class="comments__item ql-snow" v-for="(item, key) in items" :key="key">
        <div class="comments__header">
            ...
            <button class="header-right" type="button" data-bs-target="#replyModal" data-bs-toggle="modal" @click="$emit('setReply', item.id)">
        ...
        <a v-if="item.file && item.file_format != 'txt'" :href="item.file" :data-lightbox="item.file" target="_blank"
            :data-title="item.file_name" class="comments__img">
            <img :src="item.file" alt="img">
        </a>
        <a v-if="item.file && item.file_format == 'txt'" :href="item.file" target="_blank"
            class="comments__img comments__txt">
        ...
        <div class="sub-comment" v-if="item.childs">
            <comment-datails :items="item.childs" @setReply="e => $emit('setReply', e)"></comment-datails>
        </div>
    </div>
</template>
